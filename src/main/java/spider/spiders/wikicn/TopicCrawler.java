...
//		String domain = "数据结构";
		...
		int firstLayer = 1;
		List<Term> topicFirst = TopicCrawlerDAO.topic(domain_url); // 得到第一层领域术语（不含子主题）
		 ...
		 */
		int secondLayer = 2;
		List<Term> layerSecond = TopicCrawlerDAO.layer(domain_url); // 获取第一层领域术语（含子主题）
		...
		if(layerSecond.size() != 0){
			for(int i = 0; i < layerSecond.size(); i++) {
				...
				List<Term> topicSecond = TopicCrawlerDAO.topic(url); // 得到第二层领域术语（不含子主题）
				...
				int thirdLayer = 3;
				List<Term> layerThird = TopicCrawlerDAO.layer(url); // 得到第二层领域术语（含子主题）
