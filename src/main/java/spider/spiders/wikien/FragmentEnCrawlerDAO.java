 ...
 * @date 2018年3月10日
 */
public class FragmentEnCrawlerDAO {

    public static void main(String[] args) throws Exception {
        // 设置解析参数
        String topicName = "Algorithms_and_data_structures"; // 链表  跳跃列表  数据结构与算法列表
...
//                Log.log("该主题没有目录，不是目录结构，直接爬取 -->摘要<-- 信息");
    ...
    public static List<FacetSimple> getFacet(Document doc) {
        ...
        List<String> firstTitle = FragmentEnExtract.getFirstTitle(doc);
        List<String> secondTitle = FragmentEnExtract.getSecondTitle(doc);
        List<String> thirdTitle = FragmentEnExtract.getThirdTitle(doc);
    ...
    public static List<AssembleFragmentFuzhu> getFragment(Document doc) {
        ...
        if (mainContents.size() == 0) {
            // 网页全部内容
            List<AssembleFragmentFuzhu> specialContent = FragmentEnExtract.getSpecialContent(doc); // 没有目录栏的词条信息
            assembleList.addAll(specialContent);
        } else {
            // 摘要信息
            List<AssembleFragmentFuzhu> summaryContent = FragmentEnExtract.getSummary(doc); // 摘要内容
            assembleList.addAll(summaryContent);
            // flagFirst 为 true，保留一级分面数据
            LinkedList<String> firstTitle = FragmentEnExtract.getFirstTitle(doc);
            if (firstTitle.size() != 0) {
                List<AssembleFragmentFuzhu> firstContent = FragmentEnExtract.getFirstContent(doc); // 一级分面内容
                if (firstContent != null) assembleList.addAll(firstContent);
            }
            // flagSecond 为 true，保留二级分面数据
            LinkedList<String> secondTitle = FragmentEnExtract.getSecondTitle(doc);
            if (secondTitle.size() != 0) {
                List<AssembleFragmentFuzhu> secondContent = FragmentEnExtract.getSecondContent(doc); // 二级分面内容
                if (secondContent != null) assembleList.addAll(secondContent);
            }
            // flagThird 为 true，保留三级分面数据
            LinkedList<String> thirdTitle = FragmentEnExtract.getThirdTitle(doc);
            if (thirdTitle.size() != 0) {
                List<AssembleFragmentFuzhu> thirdContent = FragmentEnExtract.getThirdContent(doc); // 三级分面内容
