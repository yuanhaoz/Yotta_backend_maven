...
public class RankText {
     ...
     * @return 主题间的认知关系
     */
    public List<Dependency> rankText(List<dependency.ranktext.Term> termList, String ClassName, int MAX) {
        List<Dependency> dependencies = new ArrayList<>();

        logger.info("Finish Hash...");
        logger.info("Start computing the hammingDistance...");
        HashMap<TwoTuple<dependency.ranktext.Term, dependency.ranktext.Term>, Double> disMap = new HashMap<>();
        for (int i = 0; i < termList.size() - 1; i++) {
            for (int j = i + 1; j < termList.size(); j++) {
                dependency.ranktext.Term term1 = termList.get(i);
                dependency.ranktext.Term term2 = termList.get(j);
                double dis = CosineSimilarAlgorithm.getSimilarity(term1.getTermText(), term2.getTermText());
//				logger.info(dis+"");
                TwoTuple<dependency.ranktext.Term, dependency.ranktext.Term> twoTuple = new TwoTuple<>(term1, term2);
        ...
        List<Map.Entry<TwoTuple<dependency.ranktext.Term, dependency.ranktext.Term>, Double>> infoIds = new ArrayList<Map.Entry<TwoTuple<dependency.ranktext.Term, dependency.ranktext.Term>, Double>>(disMap.entrySet());
        ...
        logger.info("end:" + end);
        for (int k = 0; k < end; k++) {
            TwoTuple<dependency.ranktext.Term, dependency.ranktext.Term> twoTuple = infoIds.get(k).getKey();
